...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://telegra.ph/file/db9634176b0a99e425b30.jpg",
        caption=f"""REPO""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "💋REPO💋", url=f"https://github.com/ArdNeriw/ARD_ASTHETIC_MUSIC"
                    )
                ]
            ]
        ),
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "💋REPO💋", url=f"https://github.com/ArdNeriw/ARD-ASTHETIC-MUSIC"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @ARD_ASTHE_BOT`**"
        )
        return
